...
then
  ...
  then
    ...
    echo " "
    set -x
    err_exit "ERROR RUNNING supvit.x IN extrkr_gen_gfs LINE $LINENO"
  fi
...
#     300 to 900 mb, every 50 mb....
      ...
      if [ $rcc -ne 0 ]; then
        ...
        echo " "
        set -x
        err_exit "vint.x- ERROR for GPH AT extrkr_gen_gfs.sh LINE $LINENO"
      fi
...
#     300 to 500 mb, every 50 mb....
      ...
      if [ $rcc -ne 0 ]; then
        ...
        echo " "
        set -x
        err_exit "vint.x- ERROR for T AT extrkr_gen_gfs.sh LINE $LINENO"
      fi
...
#    cp ${TRKDATA}/trak.${atcfout}.atcf_gen.${regtype}.${PDY}${cyc} ${COMOUT}/
  ...
  set +x
  echo " "
  echo "FATAL ERROR:  An error occurred while running gettr.x, "
