...
then
  ...
  ${EXECens_tracker}/gfdl_supvit.x <${DATA}/suv_input.${atcfout}.${PDY}${cyc}
  suvrcc=$?
...
#     300 to 900 mb, every 50 mb....
      ...
      if [ $rcc -ne 0 ]; then
        ...
        echo " "
        set -x
        err_exit "FAILED ${jobid} -gfdl_vint.x-  for GPH AT LINE $LINENO - ABNORMAL EXIT"
      fi
...
#     300 to 500 mb, every 50 mb....
      ...
      if [ $rcc -ne 0 ]; then
        ...
        echo " "
        set -x
        err_exit "FAILED ${jobid} -gfdl_vint.x-  for T AT LINE $LINENO - ABNORMAL EXIT"
      fi
...
#    cp ${DATA}/trak.${atcfout}.atcf_gen.${regtype}.${PDY}${cyc} ${COMOUT}/
  ...
  echo " "
  set -x
  err_exit "FAILED ${jobid} - ERROR RUNNING gfdl_gettrk.x IN 2nd step- ABNORMAL EXIT"
fi
