...
#   --------------------------------------------
    ...
    do
      ...
      if [ $rcc -ne 0 ]; then
        ...
        echo " "
        set -x
        err_exit "gfdl_tave.x- ERROR AT extrkr_gfs.sh LINE $LINENO"
      fi
    ...
    if [ ${PHASEFLAG} = 'y' ]; then
      ...
      # Output file
        ...
        if [ $rcc -ne 0 ]; then
          set +x
          echo " "
          echo "FATAL ERROR in call to gfdl_tave.x at fhour= $fhour for $cmodel $pert "
          echo "rcc= $rcc      EXITING.... "
          echo " "
          set -x
          err_exit "gfdl_tave.x- ERROR AT extrkr_gfs.sh LINE $LINENO"          
...
#     300 to 900 mb, every 50 mb....
      ...
      if [ $rcc -ne 0 ]; then
        ...
        echo " "
        set -x
        err_exit "gfdl_vint.x- ERROR for GPH AT extrkr_gfs.sh LINE $LINENO"
      fi
...
#     300 to 500 mb, every 50 mb....
      ...
      if [ $rcc -ne 0 ]; then
        ...
        echo " "
        set -x
        err_exit "gfdl_vint.x- ERROR for T AT extrkr_gfs.sh LINE $LINENO"
      fi
